...
### Inverse Kinematics Experiments:
  ...
  ```bash
  % cd $PROJECT_ROOT && python IK_server.py
  ```

  In my implementation, IK_server.py:
  ```python
  ...
  rospy.spin()
  ```
  is a **main script** for **kuka_kinematics** package, the majority of the implementation is in ```kuka_kinematics/kuka_model.py```, which is already explained in the above sections.

  you can also start the IK server by:
  ```bash
  $ python -m kuka_kinematics --ik_server=True  
  ```
  ...
  the robot successfully picked and places 9/10 of the bins
  ...
  this is caused by the flaw in trajectory planning, the path is not completely collision free.
...
### Future Improvements:
  ...
  * better trajectory planning to avoid collision of any kind.
